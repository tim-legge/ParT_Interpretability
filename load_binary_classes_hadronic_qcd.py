...
def load_data(dataset_type='qg', start_index=None, batch_size=300):
    ...
    try:
        if dataset_type == 'qg':
            ...
            datapath = '/part-vol-3/weaver-core/particle-transformer/datasets/JetClass/Pythia/test_20M/TTBar_100.root'
            if os.path.exists(data_path):
            ...
            datapath = '/part-vol-3/weaver-core/particle-transformer/datasets/JetClass/Pythia/test_20M/ZJetsToNuNu_100.root'
            if os.path.exists(data_path):
                print(f"Loading actual QuarkGluon data from {data_path}")
                with uproot.open(data_path)['tree'] as tree:
                    print('This part is working - QG')
                    ...
                    for num_segs in range(hadronic_data['labels'].shape[0]//10):
                        ...
                        data['pf_points'] = np.append((data['pf_points'],hadronic_data['pf_points'][start_idx:end_idx]))
                        data['pf_features'] = np.append((data['pf_features'][:],hadronic_data['pf_features'][start_idx:end_idx]))
                        data['pf_vectors'] = np.append((data['pf_vectors'][:],hadronic_data['pf_vectors'][start_idx:end_idx]))
                        data['pf_mask'] = np.append((data['pf_mask'][:],hadronic_data['pf_mask'][start_idx:end_idx]))
                        data['labels'] = np.append((data['labels'][:],hadronic_data['label'][start_idx:end_idx]))

                        data['pf_points'] = np.append((data['pf_points'],qcd_data['pf_points'][start_idx:end_idx]))
                        data['pf_features'] = np.append((data['pf_features'][:],qcd_data['pf_features'][start_idx:end_idx]))
                        data['pf_vectors'] = np.append((data['pf_vectors'][:],qcd_data['pf_vectors'][start_idx:end_idx]))
                        data['pf_mask'] = np.append((data['pf_mask'][:],qcd_data['pf_mask'][start_idx:end_idx]))
                        data['labels'] = np.append((data['labels'][:],qcd_data['label'][start_idx:end_idx]))                

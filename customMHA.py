...
state_dict = torch.load('/part-vol-3/weaver-core/particle_transformer/training/JetClass/Pythia/full/ParTNoPairs/20241015-160839_example_ParticleTransformerNoPairs_ranger_lr0.001_batch128/net_best_epoch_state.pt')
base_model = get_model()[0]
...
folder_path = '/part-vol-3/weaver-core/particle_transformer/datasets/JetClass/Pythia/test_20M'
...
def inference_timer(model, dataloader, save_dir, save_interval=7800):
    model.eval()
    batch_count = 0
    save_batch_count = 0
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Processing batches"):
            x_particles = batch['x_particles'].to('cuda')
            x_jets = batch['x_jets'].to('cuda')
            y = batch['y'].to('cuda')
            x_points = batch['x_points'].to('cuda')
            x_mask = batch['x_mask'].to('cuda')

            # Perform inference
            outputs = model(x_points, x_particles, x_jets, x_mask)

            # Accumulate results in lists
            ...
            if batch_count % save_interval == 0:
                # Save accumulated outputs and labels
                np.save(os.path.join(save_dir, f'outputs_batch_{save_batch_count}.npy'), np.concatenate(outputs_list, axis=0))
                np.save(os.path.join(save_dir, f'labels_batch_{save_batch_count}.npy'), np.concatenate(labels_list, axis=0))
...
# Directory where you want to save the batch files
save_dir = '/part-vol-3/weaver-core/parTaaron/parTNoPairs'

# Perform inference and save outputs every 7800 batches
inference_timer(base_model.to('cuda'), dataloader, save_dir, save_interval=7800)

...
if not os.path.exists('/part-vol-3/timlegge-ParT-trained/vol_jc_full_data/'):
...
if counter >= 51:
    ...
    while counter < 51:
        ...
        jc_full_pf_features = jc_full_data['pf_features'][counter*batch_to_load:(counter+1)*batch_to_load]
        jc_full_pf_vectors = jc_full_data['pf_vectors'][counter*batch_to_load:(counter+1)*batch_to_load]
        jc_full_pf_mask = jc_full_data['pf_mask'][counter*batch_to_load:(counter+1)*batch_to_load]
        jc_full_pf_points = jc_full_data['pf_points'][counter*batch_to_load:(counter+1)*batch_to_load]
        jc_full_labels = jc_full_data['labels'][counter*batch_to_load:(counter+1)*batch_to_load]
        jc_full_model.eval()
        with torch.no_grad():
        ...
        jc_full_attention = [tensor.numpy() for tensor in jc_full_model.get_attention_matrix()]
        jc_full_topsonly_attention = []
        jc_full_qcdonly_attention = []
        for jet in range(len(jc_full_attention[0][:])):
            if np.argmax(jc_full_labels[jet][:]) == 8: # Hadronic Top
                jc_full_topsonly_attention.append([layer[jet] for layer in jc_full_attention])
            elif np.argmax(jc_full_labels[jet][:]) == 0: # QCD
